...
import { generateJobSuggestions, TailorInputStaleError, TailorNoContentError, TailorQuotaExceededError } from '@/lib/jobs/job-tailor'
...
export async function POST(request: Request, context: RouteContext): Promise<Response> {
  ...
  try {
    ...
    if (error instanceof TailorNoContentError) return NextResponse.json({ code: 'NO_VALID_SUGGESTIONS', error: '当前内容不足以生成可靠建议，请先补充真实经历' }, { status: 422 })
