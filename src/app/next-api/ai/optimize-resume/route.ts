  ...
  try {
    return withQuotaCheck("ai:optimize-resume", async () => {
      ...
      const filteredBlocks = blocks.filter(
        (b) =>
          b.contentHtml &&
          b.contentHtml.replace(/<[^>]*>/g, "").trim().length >=
            MIN_OPTIMIZE_CONTENT_LENGTH,
      );

      if (filteredBlocks.length === 0) {
        return NextResponse.json(
          { error: "所有模块内容过少，请先填写简历内容再使用一键优化" },
          { status: 400 },
        );
      }
      ...
      if (jobId) {
        ...
        confirmedEvidence = mergeJobEvidence(baseFacts, job.matchSnapshot)
          .map((fact) => ({
            blockId: fact.blockId,
            text: fact.text.trim().slice(0, 2000),
          }))
          .slice(0, 50);
        if (
          job.identity === "student" ||
          job.identity === "graduate" ||
          job.identity === "professional"
        ) {
