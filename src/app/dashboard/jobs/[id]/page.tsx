                  ...
                  <div className="mt-4 grid gap-3 lg:grid-cols-2">
                    {application.interviews.map((interview) => (
                      <div key={interview.id} className="rounded-xl border border-violet-100 bg-white p-4">
                        <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-medium text-slate-700">{interview.round}</p><p className="mt-1 text-xs text-slate-400">{interview.scheduledAt ? new Intl.DateTimeFormat('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }).format(interview.scheduledAt) : '时间待定'}{interview.interviewer ? ` · ${interview.interviewer}` : ''}</p></div><span className="rounded-full bg-violet-50 px-2.5 py-1 text-xs text-violet-700">{INTERVIEW_RESULT_LABEL[interview.result as InterviewResult] ?? interview.result}</span></div>
