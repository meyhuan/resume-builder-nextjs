...
import { ArrowLeft, ArrowRight, Check, FileText, Loader2, RefreshCw, ShieldCheck, Sparkles } from 'lucide-react'
...
interface ApiResponse { readonly error?: string; readonly suggestionSet?: JobSuggestionSet; readonly resumeId?: string }
...
export function TailorReview({ jobId, resumeId, suggestionSet, facts }: TailorReviewProps): ReactElement {
  ...
  if (!suggestionSet) {
    return (
      <section className="rounded-2xl border border-violet-100 bg-gradient-to-br from-white via-violet-50/60 to-fuchsia-50/60 p-8 text-center shadow-sm"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-white text-violet-600 shadow-sm"><Sparkles className="h-8 w-8" /></div><h2 className="mt-5 text-xl font-semibold text-slate-800">生成岗位专属优化建议</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-7 text-slate-500">AI 只会使用你确认的真实事实，并逐条说明修改原因、岗位关键词和事实来源。建议不会自动写入简历。</p><div className="mx-auto mt-5 flex max-w-xl items-start gap-3 rounded-xl border border-emerald-100 bg-emerald-50 p-4 text-left"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-emerald-600" /><p className="text-sm leading-6 text-emerald-800">严格写实模式已开启：禁止新增经历、工具、职责和量化结果。</p></div><Button onClick={() => generate(false)} disabled={busy} className="mt-6 rounded-lg bg-gradient-to-r from-violet-600 to-fuchsia-500 px-7 text-white hover:from-violet-700 hover:to-fuchsia-600">{busy ? <Loader2 className="animate-spin" /> : <Sparkles />}{busy ? '正在读取岗位和事实…' : '生成优化建议'}</Button></section>
    )
  }
