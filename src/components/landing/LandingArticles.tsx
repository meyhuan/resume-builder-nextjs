...
export const LandingArticles = ({
  id,
}: ArticlesSectionProps): React.ReactElement => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-base text-slate-500 max-w-2xl mx-auto">
            覆盖简历写作、面试技巧、应届求职、职场发展全流程，助你快速拿到心仪
            Offer。
          </p>
        </div>
          ...
          {articles.map((article) => {
            ...
            return (
              <Link
                ...
                className="group flex flex-col bg-white/70 backdrop-blur-md rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg hover:bg-white hover:-translate-y-0.5 transition-all duration-300 overflow-hidden h-[210px]"
              >
                <div className="flex flex-col flex-1 p-4">
                  {/* Category + Date */}
                  <div className="flex items-center justify-between mb-3">
                    <span
                      className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full ${catMeta.bgColor} ${catMeta.color}`}
                    >
                      {catMeta.label}
                    </span>
