...
const DEMO_STEPS = [
  {
    ...
    title: "AI 智能生成",
    description:
      "输入你的基本信息，AI 自动生成专业简历内容，零经验也能写出好简历。",
    color: "violet",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {DEMO_STEPS.map((step, index) => (
            <div
              key={index}
              className="group relative p-5 rounded-2xl border border-slate-100 bg-white hover:shadow-xl hover:shadow-violet-500/5 hover:-translate-y-1 transition-all duration-500"
            >
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 transition-colors duration-300 ${step.bgClass}`}
              >
                {step.icon}
              </div>
