  ...
  <div className="mt-3 flex gap-3 flex-1 min-h-0">
    ...
    <div className="flex-1 bg-white/60 backdrop-blur-sm rounded-xl border border-white shadow-sm p-3 space-y-2 flex flex-col">
      ...
      {[
        ...
        <div key={field.label} className="space-y-0.5">
          ...
          <div className="h-6 bg-white rounded-md border border-slate-100 px-2 flex items-center text-[10px] text-slate-700 font-medium">
            {field.value}
          </div>
        </div>
      ))}
    ...
    <div className="flex-1 bg-white/60 backdrop-blur-sm rounded-xl border border-white shadow-sm p-3 space-y-2 flex flex-col">
      ...
      <div className="flex items-center gap-2">
        <div className="w-6 h-6 rounded-full bg-[#8B5CF6] flex items-center justify-center text-white text-[9px] font-bold shrink-0">
          张
        </div>
      ...
      {["w-full", "w-[90%]", "w-[75%]", "w-full", "w-[85%]"].map((w, i) => (
        <div key={i} className="flex items-center gap-1.5">
          ...
          <div
            className={`h-1.5 ${w} bg-slate-100 rounded animate-pulse`}
      ...
      <div className="flex flex-wrap gap-1 mt-auto pt-1">
        {["React", "TS", "Vue"].map((tag) => (
          <span
            key={tag}
            className="text-[8px] px-1.5 py-0.5 rounded bg-[#8B5CF6]/10 text-[#8B5CF6] font-medium"
          >
            {tag}
          </span>
        ))}
...
const PolishVisual = (): React.ReactElement => (
  <div className="mt-3 space-y-2">
    <div className="bg-rose-50/80 rounded-lg px-3 py-1 border border-rose-100">
      ...
      <div className="text-[11px] text-slate-500 line-through decoration-rose-300">
        负责项目前端开发工作
      </div>
    </div>
...
const AtsVisual = (): React.ReactElement => (
  <div className="mt-3 space-y-2">
    {["PDF 格式正确", "关键词覆盖", "结构化信息"].map((item) => (
      <div
        key={item}
        className="flex items-center gap-2 bg-white/60 backdrop-blur-sm rounded-lg px-3 py-1.5 border border-white shadow-sm"
      >
  ...
  <div className="mt-3 space-y-2">
    <div className="flex flex-wrap gap-1.5">
      {["豆包", "ChatGPT", "DeepSeek", "Kimi", "通义千问"].map((name) => (
        <span
          key={name}
          className="text-[9px] font-semibold px-2 py-0.5 rounded-md bg-white/60 border border-white shadow-sm text-slate-600"
        >
          {name}
        </span>
      ))}
...
/** Feature card visual components mapped by index */
const FEATURE_VISUALS: readonly React.FC[] = [
  AiGenerateVisual,
  PolishVisual,
  MatchVisual,
  AtsVisual,
...
const AI_FEATURES: readonly FeatureItem[] = [
  {
    title: "AI 一键生成",
    description:
      "根据你的身份（在校生/应届生/职场人）和目标岗位，AI 自动调整引导流程，生成完整、专业的简历内容。",
  ...
  },
  {
    title: "AI 文本转简历",
    description:
      "粘贴文本内容，AI 自动解析结构并应用专业模板排版，支持豆包、ChatGPT 等来源。",
...
export const LandingFeatures = ({
  ...
  return (
    <section
      id={id}
      className="py-14 md:py-16 bg-[#F8FAFC] relative overflow-hidden"
    >
      {/* Background orbs */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-10 max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/80 backdrop-blur-sm border border-white rounded-full shadow-sm mb-4">
        ...
        {/* Bento Grid */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 auto-rows-[190px] md:auto-rows-[190px]">
          {AI_FEATURES.map((feature, index) => {
            ...
            return (
              <div
                key={feature.title}
                className={`group relative rounded-2xl border border-white bg-white/70 backdrop-blur-md shadow-sm hover:shadow-md hover:bg-white/90 transition-all duration-200 overflow-hidden flex flex-col ${feature.span} ${isHero ? "p-5" : "p-4"}`}
              >
                {/* Icon + Title */}
                <div className="flex items-center gap-3 mb-1">
                  <div
                    className={`w-9 h-9 rounded-xl flex items-center justify-center ${feature.bgColor} ${feature.color} shrink-0`}
                  >
                    {feature.icon}
                  </div>
                  <h3
                    className={`font-bold text-slate-800 ${isHero ? "text-xl" : "text-base"}`}
                  >
                    {feature.title}
                  </h3>
                </div>
                {/* Description */}
                <p
                  className={`text-slate-500 leading-relaxed ${isHero ? "text-sm" : "text-xs"}`}
                >
                  {feature.description}
                </p>
                {/* Visual illustration */}
