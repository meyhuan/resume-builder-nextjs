...
export interface CampusRendererProps extends BaseRendererProps {
 ...
 * 校园经历渲染器 - 支持多种视觉风格
 */
export default function CampusRenderer(props: CampusRendererProps): ReactElement {
    ...
    return (
      <div className="bg-white rounded-2xl shadow-md p-5 hover:shadow-lg transition-shadow border-l-4" style={{ borderColor: themeColor }}>
        <div className="flex justify-between items-start mb-2">
          ...
          >
            <EditableFieldWrapper blockId={block.id} fieldName="startDate" value={block.startDate} onUpdate={(): void => {}} />
            {' - '}
            <EditableFieldWrapper blockId={block.id} fieldName="endDate" value={block.endDate} onUpdate={(): void => {}} />
          </div>
        </div>
