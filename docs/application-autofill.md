...
- 京东招聘（`zhaopin.jd.com`）：通过页面字段 ID 映射个人信息、求职意向、工作经历、项目经历、教育经历、技能、证书和个人网站。页面里的 `nationality` 实际对应“民族”，已通过站点规则纠正，避免误填为国籍。
- 腾讯招聘（`careers.tencent.com`）：只扫描简历主体，排除顶部搜索框、电话国家区号、国家选择框和 Element UI 地区搜索框；支持姓名、当前城市、手机、邮箱、工作职位、公司、工作描述、学校、专业和自我评价。
...
- 不自动新增教育、工作或项目经历行。
- 暂不操作腾讯的国家/期望城市等自定义选择器，也不操作京东的只读日期选择器。
